{include file="admin/page_header_admin.tpl" title='Cupones de descuento'}
<div class="d-flex flex-column align-items-center justify-content-center default-space">
    
    <div class="fw-bold title text-warning">Cupones de descuentos</div>
    ...
    <section class="bg-light col-12 col-lg-7 section-slim">
        <form class="d-flex d-wrap flex-wrap mb-2 justify-content-center align-items-center">
            <label for=" creator" class="col-sm-2 col-form-label w-auto me-1">Lista de cupones generados por:</label>
        ...
        <div>
            ...
            {else}
                <button id="toggleAll" class="btn btn-outline-dark btn-sm mb-2 w-100" onclick="toggleAllCollapses()">
                    ...
                    Mostrar/ocultar detalle de todos los cupones
                </button>
                {foreach from = $discountCps item = $dc}
                    {include file="admin/card_discount.tpl" discount=$dc}
                {/foreach}
            {/if}
        </div>
